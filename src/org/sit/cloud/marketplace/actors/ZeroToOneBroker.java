...
import migrdecidernorm.migrdecidernorm;
...
public class ZeroToOneBroker {
	...
	private migrdecidernorm theMigrationNorm;

	private Map<String, Long> vmIdToCreationTimeMap;

	private Map<String, Long> vmIdToMigrationTimeMap;
	...
	public void registerProvider(Provider provider){
		...
		availabilityTrustMap.put(provider.getId(), 1.0);
		bandwidthTrustMap.put(provider.getId(), 1.0);
	}
	...
	private void insertTrustValuesInProviderParams(List<ProviderParams> providerParams){
		for(ProviderParams providerParam : providerParams){
			providerParam.setTrustInAvailability(availabilityTrustMap.get(providerParam.getProviderId())*1.6);
			providerParam.setTrustInBandwidth(bandwidthTrustMap.get(providerParam.getProviderId())*1.6);
		}
	}
	...
	public void printFValues(){
		for(String vmId : availabilitySatisfactionMap.keySet()){
			System.out.println("VM ID" + "\t" + vmId + "\t" + availabilitySatisfactionMap.get(vmId));
		}
	}
	...
	private void evaluateCosts(Map<String, SlaViolationData> slaViolMap){
		for(String vmId : vmIdToCostMap.keySet()){
			...
			vmIdToAvailMap.put(vmId, vmIdToAvailMap.get(vmId)+slaViolMap.get(vmId).getExperiencedAvailability());
				...
				System.out.println("Null Pointer Exception caused by VM ID : "+vmId);
				System.out.println(vmIdToAvailMap.get(vmId));
				System.out.println(slaViolMap.get(vmId));
				System.exit(0);
			}
	...
	private void performNecessaryMigrations() throws MWException{
		if(vmIdToMigrationValue.keySet().size() > 0){
			...
			for(String vmId : vmIdToMigrationValue.keySet()){
				satisfactionValues[i][0] = availabilitySatisfactionMap.get(vmId)*1.6;
				if(satisfactionValues[i][0] > 1.6){
					System.out.println("AVAIL OUT OF RANGE");
					System.exit(0);
				}
				satisfactionValues[i][1] = bandwidthSatisfactionMap.get(vmId)*1.6;
			...
			//y=theMigrationNorm.migration_norm(1,x);
	...
	private List<ProviderParams> selectPossibleProviders(List<ProviderParams> providerParams, double requestedAvailability, double requestedBandwidth, double currentProviderAvailTrust, double currentProviderBwTrust){
		...
		for(ProviderParams param : providerParams){
			if(param.getAvailability() >= requestedAvailability && param.getBw()>=requestedBandwidth && 
					availabilityTrustMap.get(param.getProviderId())*1.6>=currentProviderAvailTrust && 
					bandwidthTrustMap.get(param.getProviderId())*1.6>=currentProviderBwTrust){
				param.setTrustInAvailability(availabilityTrustMap.get(param.getProviderId())*1.6);
				param.setTrustInBandwidth(bandwidthTrustMap.get(param.getProviderId())*1.6);
	...
	private void migrateVm(String vmId) throws MWException{
		if(vmIdToMigrationTimeMap.get(vmId)==null)
			vmIdToMigrationTimeMap.put(vmId, TimeKeeper.getTime());
		...
		List<ProviderParams> possibleTargetProviders = selectPossibleProviders(getProviderParams(), 
				...
				availabilityTrustMap.get(currentProviderId)*1.6, 
				bandwidthTrustMap.get(currentProviderId)*1.6);
		...
		String providerId = migrationDecisionMaker.selectTargetProviderForMigration(currentProviderId, availabilityTrustMap.get(currentProviderId)*1.6, 
				bandwidthTrustMap.get(currentProviderId)*1.6, registry.getProviderIdtoProviderMap().get(currentProviderId).getPromisedAvailability(),
		...
		if(TimeKeeper.shouldViolationsBeCalculated()){
			...
			for(String vmId : vmIdToSlaViolationMap.keySet()){
				...
				availabilitySatisfactionMap.put(vmId, Math.min((1-ALPHA)*sumOfExperiencedAvailabilityMap.get(vmId)/
						...
						+ availabilitySatisfactionMap.get(vmId)*ALPHA, 1.0));
				bandwidthSatisfactionMap.put(vmId, Math.min((1-ALPHA)*sumOfExperiencedBandwidthMap.get(vmId)/
	...
	public void printSimulationDetailsWrtUserReq(){
		...
		System.out.println("\n\n\n");
		printMigrationTimings();
		printInitialAndFinalProviders();
	}
	
	
	private void printMigrationTimings(){
		for(String vmId : vmIdToCreationTimeMap.keySet()){
			Long migrTime = vmIdToMigrationTimeMap.get(vmId);
			if(migrTime != null){
				//if(vmIdToCreationTimeMap.get(vmId) <= TimeKeeper.VIOLATION_START)
				//	System.out.println("VM ID\t"+ vmId + "\t:\t" + (migrTime - TimeKeeper.VIOLATION_START)/(24.0*7));
				//else
					System.out.println("VM ID\t"+ vmId + "\t:\t" + (migrTime - vmIdToCreationTimeMap.get(vmId))/(24.0*7));
			}
		}
	}
	
	private void printInitialAndFinalProviders(){
		System.out.println("VM ID\tINITIAL\tFINAL");
		for(String vmId : vmIdToInitialProviderIdMap.keySet()){
			System.out.println(vmId+"\t"+vmIdToInitialProviderIdMap.get(vmId)+"\t"+registry.getVmIdToVmMap().get(vmId).getProviderId());
		}
	}
